...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { complaint_id, escalation_reason } = await req.json();
    if (!complaint_id || !escalation_reason)
      throw new Error("complaint_id and escalation_reason are required");

    const dedupeKey = `escalation_${complaint_id}`;
    ...
    const emailRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [profile.email],
        subject: `Complaint Escalated: ${complaint.reference_id || complaint.subject}`,
        html: `
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <h2 style="color: #1a1a1a;">Complaint Escalated</h2>
            <p>Hi ${profile.display_name || "there"},</p>
            <p>Your complaint has been escalated to the Head of Department for further review.</p>
            <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
              ...
              <tr><td style="padding: 8px; font-weight: bold; color: #666;">Status</td><td style="padding: 8px;">Escalated</td></tr>
            </table>
            <div style="background: #fffbeb; border-left: 4px solid #f59e0b; padding: 16px; margin: 16px 0; border-radius: 4px;">
              <p style="margin: 0; font-weight: bold; color: #92400e;">Reason for escalation:</p>
              <p style="margin: 8px 0 0 0; white-space: pre-wrap;">${escalation_reason}</p>
            </div>
